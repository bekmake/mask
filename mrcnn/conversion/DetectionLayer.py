...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    """
    with tf.name_scope("Refine Detection Graph"):
        # Class IDs per ROI
        ...
        refined_rois = ProposalLayer.apply_box_deltas_graph(
            rois, deltas_specific * config.BBOX_STD_DEV)
        # Clip boxes to image window
        refined_rois = ProposalLayer.clip_boxes_graph(refined_rois, window)
...
class DetectionLayer:
    ...
    def call(self, inputs):
        ...
        m = DataFormatting.parse_image_meta_graph(image_meta)
        image_shape = m['image_shape'][0]
        window = DataFormatting.norm_boxes_graph(m['window'], image_shape[:2])
